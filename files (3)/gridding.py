...
class GridAccumulator:
    ...
    def to_dataset(self) -> "xr.Dataset":
        """Convertit la grille en xarray.Dataset avec coordonnées lat/lon.

        Le Dataset contient pour chaque paramètre trois variables :
        - ``{param}_mean``  : moyenne par cellule
        - ``{param}_std``   : écart-type par cellule
        - ``{param}_count`` : nombre de mesures par cellule

        Returns
        -------
        xr.Dataset  avec dimensions (lat, lon) et attributs de métadonnées
        """
        means  = self.mean()
        stds   = self.std()
        counts = self.count()
        ...
        data_vars = {}
        for param in means:
            data_vars[f"{param}_mean"]  = (["lat", "lon"], means[param])
            data_vars[f"{param}_std"]   = (["lat", "lon"], stds[param])
            data_vars[f"{param}_count"] = (["lat", "lon"], counts[param].astype(float))

        ds = xr.Dataset(
            data_vars,
            coords={
                "lat": ("lat", self.lat_bins, {"units": "degrees_north", "long_name": "latitude"}),
                "lon": ("lon", self.lon_bins, {"units": "degrees_east",  "long_name": "longitude"}),
            },
            attrs={
                "title":       "EarthCARE ACM_CLP_2B gridded statistics",
                "dlat":        self.dlat,
                "dlon":        self.dlon,
        ...
        # Attributs par variable
        _var_attrs = {
            "iwp":               {"long_name": "Ice Water Path",          "units": "g/m2"},
            "lwp":               {"long_name": "Liquid Water Path",       "units": "g/m2"},
            "surface_elevation": {"long_name": "Surface elevation",       "units": "m"},
            "iwc":               {"long_name": "Ice Water Content",       "units": "g/m3"},
            "lwc":               {"long_name": "Liquid Water Content",    "units": "g/m3"},
            "temperature":       {"long_name": "Temperature (col. mean)", "units": "K"},
            "particle_type":     {"long_name": "Dominant particle type",  "units": "1"},
        }
        for param, attrs in _var_attrs.items():
            for suffix in ("mean", "std", "count"):
                vname = f"{param}_{suffix}"
                if vname in ds:
                    ds[vname].attrs.update(attrs)
                    if suffix == "std":
                        ds[vname].attrs["long_name"] += " — std dev"
                    elif suffix == "count":
                        ds[vname].attrs = {"long_name": f"{attrs['long_name']} — sample count", "units": "1"}
        return ds

    def save(self, filepath: str) -> None:
        """Sauvegarde la grille en NetCDF4 (.nc) via xarray.

        Le fichier peut être rechargé instantanément avec
        ``GridAccumulator.load(filepath)`` ou directement avec
        ``xr.open_dataset(filepath)`` sans passer par ce module.

        Parameters
        ----------
        filepath : str
            Chemin de destination. L'extension .nc est recommandée.
        """
        # Sauvegarder aussi les accumulateurs bruts (sum, sum2, count)
        # pour permettre de reprendre l'accumulation depuis le fichier.
        ds = self.to_dataset()

        # Ajouter les accumulateurs bruts comme variables cachées
        for param in self._sum:
            ds[f"_sum_{param}"]   = (["lat", "lon"], self._sum[param])
            ds[f"_sum2_{param}"]  = (["lat", "lon"], self._sum2[param])
            ds[f"_count_{param}"] = (["lat", "lon"], self._count[param].astype(float))

        Path(filepath).parent.mkdir(parents=True, exist_ok=True)
        ds.to_netcdf(filepath, mode="w")
        size_mb = Path(filepath).stat().st_size / 1e6
        print(f"[grid] Sauvegardé → {filepath}  ({size_mb:.1f} MB, "
              f"{self.n_orbits} orbites, {self.n_lat}×{self.n_lon} cellules)")

    @classmethod
    def load(cls, filepath: str) -> "GridAccumulator":
        """Charge une grille depuis un fichier NetCDF4 (.nc).
        ...
        Returns
        -------
        GridAccumulator  avec l'état complet restauré (accumulation reprennable)
        """
        ...
        g.n_orbits = int(ds.attrs["n_orbits"])
        g._sum     = {}
        g._sum2    = {}
        g._count   = {}
        ...
        for param in all_params:
            g._sum[param]   = ds[f"_sum_{param}"].values.copy()
            g._sum2[param]  = ds[f"_sum2_{param}"].values.copy()
            g._count[param] = ds[f"_count_{param}"].values.astype(np.int32).copy()

        ds.close()
        print(f"[grid] Chargé ← {filepath}  "
              f"({g.n_orbits} orbites, {g.n_lat}×{g.n_lon} cellules)")
        return g
