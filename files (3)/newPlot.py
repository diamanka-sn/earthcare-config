...
def _filter_time_window(orb: dict,
                         t_min: float = T_MIN,
                         t_max: float = T_MAX) -> dict | None:
    """Filtre les points d une orbite hors de la fenetre de temps [t_min, t_max].

    Le tableau time de chaque orbite est relatif au debut du passage
    (t[0] = 0). On calcule t_rel = time - time[0] et on garde uniquement
    les points dans [t_min, t_max].

    Parameters
    ----------
    t_min, t_max : float
        Bornes en secondes depuis le debut de l orbite (depuis config.py).

    Returns
    -------
    dict avec les memes cles mais tronque aux points valides,
    ou None si aucun point ne tombe dans la fenetre.
    """
    t = orb.get("time")
    if t is None:
        return orb   # pas de temps : on garde tel quel

    t_rel  = np.asarray(t, dtype=float)
    t_rel  = t_rel - t_rel[0]          # secondes depuis debut orbite
    ...
    for key, val in orb.items():
        arr = np.asarray(val) if isinstance(val, (list, np.ndarray)) else val
        if isinstance(arr, np.ndarray) and arr.ndim >= 1 and len(arr) == len(mask):
...
def plot_multi_orbit_lwp(orbites: list[dict], n_orbites_label: str) -> None:
    ...
    sc_lwp = None
    for orb in orbites:
        orb_w = _filter_time_window(orb)
        if orb_w is None:
            continue
        sc_lwp = ax.scatter(orb_w["lon"], orb_w["lat"], c=orb_w["lwp"],
                             cmap="rainbow", s=5,
...
def plot_orbits_by_period(orbites: list[dict],
    ...
    for idx in range(n_periods):
        ...
        for orb in period_orbits:
            ...
            if orb_w is None:
                continue
            values = orb_w.get(param)
            if values is None:
                continue
            values = np.where(values < 0, np.nan, values)
            sc = ax.scatter(
                orb_w["lon"], orb_w["lat"], c=values,
                cmap=cmap, s=5,
...
def orbites_above_threshold(orbites: list[dict],
    ...
    if plot:
        ...
        # Seuls les points dans la fenetre de temps, valeurs > seuil
        sc = None
        for orb in filtered:
            orb_w = _filter_time_window(orb)
            if orb_w is None:
                continue
            values = np.asarray(orb_w.get(param, []), dtype=float)
            values = np.where(values < 0, np.nan, values)
            # Masquer les points sous le seuil
            values_plot = np.where(values > threshold, values, np.nan)
            sc = ax.scatter(orb_w["lon"], orb_w["lat"],
                            c=values_plot, cmap="rainbow", s=6,
...
def plot_grid_count(grid_or_dict,
                    ...
                    d2: str | None = None,
                    log_scale: bool = True) -> None:
    """Carte polaire du nombre de mesures par cellule (count).

    Fonctionne avec DailyGridAccumulator ou directement avec un dict
    retourne par count() / count_range().

    Parameters
    ----------
    grid_or_dict : DailyGridAccumulator ou dict {param: ndarray}
    param : str
        Parametre a afficher (ex. "lwp", "iwp").
    day : str, optional
        Jour au format "YYYY-MM-DD". Prioritaire sur d1/d2.
    d1, d2 : str, optional
        Plage de dates "YYYY-MM-DD". Utilise si day est None.
    log_scale : bool, default True
        Echelle logarithmique sur la colorbar (recommande car les
        comptes peuvent varier de 1 a plusieurs milliers).

    Example
    -------
    >>> plot_grid_count(grid, param="lwp", day="2025-12-30")
    >>> plot_grid_count(grid, param="iwp", d1="2025-12-01", d2="2026-01-31")
    """
    import matplotlib.colors as mcolors

    # --- Recuperer le tableau count ---
    if isinstance(grid_or_dict, dict):
        # On a deja un dict de counts
        counts = grid_or_dict
        lon_bins  = None
        lat_bins  = None
        n_orbits  = None
    ...
    data   = np.where(data == 0, np.nan, data)   # masquer les cellules vides
    ...
    title = param.upper() + " — nombre de mesures par cellule  (" + label + ")"
    header = "ECA_JXBA_ACM_CLP_2B — count  " + label
    if n_orbits is not None:
        header += "  (" + str(n_orbits) + " orbites)"
    ...
    cbar = plt.colorbar(pc, ax=ax, orientation="vertical", shrink=0.7, pad=0.05)
    cbar.set_label("Nombre de mesures" + (" (echelle log)" if log_scale else ""),
                   fontsize=10)
...
def plot_grid_count_histogram(grid_or_dict,
                               param: str = "lwp",
                               day: str | None = None,
                               d1: str | None = None,
                               d2: str | None = None) -> None:
    """Histogramme de la distribution du nombre de mesures par cellule.

    Complemente plot_grid_count() en montrant la distribution globale
    des counts plutot que leur position geographique.

    Parameters
    ----------
    Memes parametres que plot_grid_count().

    Example
    -------
    >>> plot_grid_count_histogram(grid, param="lwp", day="2025-12-30")
    """
    import pandas as pd

    # --- Recuperer les counts ---
    if isinstance(grid_or_dict, dict):
        counts = grid_or_dict
        label  = param
    else:
    ...
    data  = data[data > 0]   # garder seulement les cellules non vides
    ...
    fig.patch.set_facecolor("white")
    fig.suptitle(
        param.upper() + " — distribution du nombre de mesures par cellule  ("
        + label + ")",
        fontsize=12, fontweight=TITLE_WEIGHT, color=TITLE_COLOR,
    )

    # Histogramme lineaire
    ax1.hist(data, bins=40, color=TITLE_COLOR, edgecolor="white",
             linewidth=0.3, alpha=0.85)
    ax1.set_xlabel("Nombre de mesures", fontsize=10)
    ax1.set_ylabel("Nombre de cellules", fontsize=10)
    ax1.set_title("Echelle lineaire", fontsize=10)

    # Statistiques
    stats = pd.Series(data)
    txt = ("n cellules = " + str(len(data)) + "\n"
    ...
    # Histogramme log
    ax2.hist(data, bins=40, color=TITLE_COLOR, edgecolor="white",
             linewidth=0.3, alpha=0.85, log=True)
    ax2.set_xlabel("Nombre de mesures", fontsize=10)
    ax2.set_ylabel("Nombre de cellules (log)", fontsize=10)
